...
# through it, asserting that the persisted object graph is initialized exactly once per
# worker and that its frozen state - root, nested object and the cycle between them -
# survives every RINIT/RSHUTDOWN boundary.
#
...
BAD_CHILDREN=$(grep -vc 'child=child-marker' "$OUT" || true)
BAD_CYCLES=$(grep -vc 'cycle=1' "$OUT" || true)

echo "requests=$REQUESTS results=$RESULTS errors=$ERRORS workers=$PIDS inits=$INITS bad_markers=$BAD_MARKERS bad_ports=$BAD_PORTS bad_children=$BAD_CHILDREN bad_cycles=$BAD_CYCLES"
...
[ "$BAD_CHILDREN" -eq 0 ] || { echo "FAIL: a request observed a mutated nested object - graph rollback is broken"; exit 1; }
[ "$BAD_CYCLES" -eq 0 ] || { echo "FAIL: the persisted graph cycle did not survive the request boundary"; exit 1; }

echo "GATE OK: object graph survived $((REQUESTS - 1)) real request boundaries in one worker"
